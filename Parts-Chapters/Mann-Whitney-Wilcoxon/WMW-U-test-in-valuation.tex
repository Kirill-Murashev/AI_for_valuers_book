...
After that, we proceed to calculate the standardized value (z-score) by formula~\ref{eq:z-score}. To do this in cell C37, calculate the mean using formula~\ref{eq:U-mean}. Then we move on to the question of calculating the standard deviation. It should be noted that there are two formulas for this. The first~(\ref{eq:standard-deviation-no-ties}) applies if there are no ties (cell C38). The second~(\ref{eq:standard-deviation-ties}) applies when they are present (cell C39). In the case at hand, ties were present. They were processed in columns~P and~Q, as well as in cells~E35:E49.
1173
